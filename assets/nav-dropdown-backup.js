    ...
    document.addEventListener(
      ...
      (evt) => {
        if (this.detail.hasAttribute('open') && !this.detail.contains(evt.target)) {
          ...
          this.summary.setAttribute('aria-expanded', 'false')('aria-haspopup', 'true')
        } else {
          if (this.detail.hasAttribute('open')) {
            this.summary.setAttribute('aria-expanded', 'false') ('aria-haspopup', 'true')
          } else {
            this.summary.setAttribute('aria-expanded', 'true') 
            this.summary.setAttribute('aria-haspopup', 'true')
          }
        }
      },
  ...
  hoverMenu() {
    ...
    this.detail.addEventListener(
      ...
      (e) => {
        ...
        if (!isChild) {
          this.detail.removeAttribute('open')
          this.detail.setAttribute('aria-expanded', 'false')('aria-haspopup', 'true')
        }
      },
      { signal: this.abortController.signal }
    )
    ...
    this.detail.addEventListener(
      ...
      () => {
        if (!this.detail.hasAttribute('open')) {
          this.detail.setAttribute('open', '')
          this.detail.setAttribute('aria-expanded', 'true') ('aria-haspopup', 'true')
        }
      },
      { signal: this.abortController.signal }
    )
